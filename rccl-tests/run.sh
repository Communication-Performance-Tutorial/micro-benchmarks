...
#       INTER_CCD    — local indices 0,2  (diff CCD, same die)  ← collected
...
if [ -n "${CPX_MODE}" ]; then
    ...
    # CPX mode adds only the INTER_CCD data point (same die, different CCDs).
    #
    ...
    echo "  NOTE: INTRA_CCD and INTER_CCD sendrecv require CPX-mode nodes."
