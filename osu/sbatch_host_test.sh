...
if [[ -f "$SUBMIT_DIR/build.sh" && -f "$SUBMIT_DIR/run_host.sh" ]]; then
    SCRIPT_DIR="$SUBMIT_DIR"
elif [[ -f "$SUBMIT_DIR/osu/build.sh" && -f "$SUBMIT_DIR/osu/run_host.sh" ]]; then
    SCRIPT_DIR="$SUBMIT_DIR/osu"
else
    SCRIPT_DIR=""
    for candidate in "$SUBMIT_DIR"/*/micro-benchmarks/osu; do
        if [[ -f "$candidate/build.sh" && -f "$candidate/run_host.sh" ]]; then
