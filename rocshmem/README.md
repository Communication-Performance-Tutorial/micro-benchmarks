...
> `INTER_XCD` test) — they are the same cross-package (`INTER_SOCKET`-class) GPU
> communication as the third mode, differing **only in CPU pinning**, not in
> which GPUs — or which packages — actually communicate.  It is not possible to
> assign both ranks to the same GPU, nor to two XCDs on the *same* package, using
> this test binary in SPX mode; that would require CPX-mode partitioning.
>
